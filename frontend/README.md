...
Esto crea los siguientes servicios:
...
Puedes comprobar que estén activos con:

```bash
docker compose ps
```

> El `docker-compose.yml` actual levanta la infraestructura de datos. Backend y frontend se ejecutan con npm en los pasos siguientes.
...
Después de clonar el proyecto, el flujo completo puede resumirse así:
